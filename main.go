...
func versionUpdate(repository string, debpack string, path string)  {
	var version Version
	...
	json.Unmarshal(file, &version)

	version.Previous = version.Latest

	version.Latest = debpack
...
func uploadHandler(config conf, db *bolt.DB) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		for {
			...
			versionUpdate(repository, part.FileName(), filepath.Join(config.ArchPath(distroName, section, archType)))
			if err != nil {
